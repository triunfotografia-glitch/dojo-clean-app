...
export default function Treinos() {
  ...
  function salvar() {
    ...
    diasSelecionados.forEach((dia, indice) => {
  ...
  function confirmarExclusao(id: string) {
    Alert.alert('Excluir treino', 'Deseja excluir este treino?', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Excluir', style: 'destructive', onPress: () => excluirTreino(id) }]);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={treinos} keyExtractor={(item) => item.id} ListEmptyComponent={<Text style={styles.empty}>Nenhum treino cadastrado.</Text>} renderItem={({ item }) => (
        <View style={styles.card}>
          ...
          <View style={styles.actions}><Pressable style={styles.editButton} onPress={() => router.push({ pathname: '/treino/editar/[id]', params: { id: item.id } })}><Text style={styles.editText}>Editar</Text></Pressable><Pressable style={styles.deleteButton} onPress={() => confirmarExclusao(item.id)}><Text style={styles.deleteText}>Apagar</Text></Pressable></View>
        </View>
      )} />
    </View>
  );
...
const styles = StyleSheet.create({
  container: { backgroundColor: COLORS.background, flex: 1, padding: 25, paddingTop: 70 }, title: { color: COLORS.white, fontSize: 32, fontWeight: 'bold', marginBottom: 20 }, input: { backgroundColor: COLORS.card, borderColor: COLORS.border, borderRadius: 14, borderWidth: 1, color: COLORS.white, marginBottom: 12, padding: 14 }, label: { color: COLORS.white, fontWeight: 'bold', marginBottom: 8 }, options: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 12 }, option: { backgroundColor: COLORS.card, borderColor: COLORS.border, borderRadius: 14, borderWidth: 1, marginBottom: 8, marginRight: 8, padding: 10 }, selected: { backgroundColor: COLORS.primary, borderColor: COLORS.primary }, optionText: { color: COLORS.white }, button: { alignItems: 'center', backgroundColor: COLORS.primary, borderRadius: 14, marginBottom: 20, padding: 15 }, buttonText: { color: COLORS.white, fontWeight: 'bold' }, card: { backgroundColor: COLORS.card, borderColor: COLORS.border, borderRadius: 15, borderWidth: 1, marginBottom: 10, padding: 15 }, name: { color: COLORS.white, fontSize: 18, fontWeight: 'bold' }, info: { color: COLORS.textSecondary, marginTop: 5 }, actions: { flexDirection: 'row', gap: 10, marginTop: 15 }, editButton: { alignItems: 'center', backgroundColor: COLORS.primary, borderRadius: 10, flex: 1, padding: 10 }, editText: { color: COLORS.white, fontWeight: 'bold' }, deleteButton: { alignItems: 'center', borderColor: '#E53935', borderRadius: 10, borderWidth: 1, flex: 1, padding: 10 }, deleteText: { color: '#FF6B6B', fontWeight: 'bold' }, empty: { color: COLORS.muted, marginTop: 25, textAlign: 'center' },
});
