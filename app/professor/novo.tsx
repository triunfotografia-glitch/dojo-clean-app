...
export default function NovoProfessor() {
  ...
  function salvar() {
    ...
    Alert.alert('Sucesso', `Professor cadastrado!${alunoVinculado ? `\nVinculado automaticamente ao aluno ${alunoVinculado.nome}.` : ''}`);
    router.back();
  }
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Pressable style={styles.button} onPress={salvar}><Text style={styles.buttonText}>Salvar professor</Text></Pressable>
    </ScrollView>
    </KeyboardAvoidingView>
  );
}
...
const styles = StyleSheet.create({
  ...
  label: { color: COLORS.white, fontWeight: 'bold', marginBottom: 10, marginTop: 10 },
  helperText: { color: COLORS.muted, marginBottom: 15 },
  options: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 10 },
  option: { backgroundColor: COLORS.card, borderColor: COLORS.border, borderRadius: 12, borderWidth: 1, padding: 12, marginRight: 10, marginBottom: 10 },
  ...
  button: { alignItems: 'center', backgroundColor: COLORS.primary, borderRadius: 15, marginTop: 10, padding: 17 },
